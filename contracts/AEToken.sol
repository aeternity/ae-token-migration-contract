  ...
  event WTF(bytes data);
    ...
    function approveAndCall(address _spender, uint256 _value, bytes _extraData) public returns (bool success)  {
        ...
        // one would use vanilla approve instead.
				require(_spender.call(bytes4(sha3("receiveApproval(address,uint256,address,bytes)")), msg.sender, _value, this, _extraData));
        return true;
    }
}
...
contract AEToken is PrefilledToken {
  ...
  function AEToken() HumanStandardToken(0, "Aeternity", 18, "AE") {
    uint nYears = 2;
    balances[msg.sender] = 100 * 10 ** 18; // FOR TESTING ! 
